...
|Sensor Web Enablement (SWE)|Virtual|Y|N|Attributes|Y|

|GML|Virtual|Y|Y|Attributes. Direction only (no roll)|Y|v3.2.1

|CityGML|Both|Y|Y|Y (?)|Y|
...
|Observations and Measurements|Virtual|Y|N|Attributes|Y|

|SensorThings API|Virtual|Y|Y|Attributes|Y|

|IMDF|Both|Yes|Yes|Attributes|Y|
...
The core capabilities of the E57 format link:http://libe57.org/features.html[includes fifteen features.]
